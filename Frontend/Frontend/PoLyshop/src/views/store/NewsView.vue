...
const fetchNews = async () => {
  try {
    ...
    // Transform data để khớp với template
    let articles = [];
    if (Array.isArray(data)) {
      articles = data.map(item => ({
        ...item,
        image: item.thumbnail || 'https://via.placeholder.com/600x400', // Map thumbnail thành image
        date: item.date // Giữ nguyên
      }));
    } else if (typeof data === 'object' && data !== null) {
      if (data.$values && Array.isArray(data.$values)) {
        articles = data.$values.map(item => ({
          ...item,
          image: item.thumbnail || 'https://via.placeholder.com/600x400',
          date: item.date
        }));
      } else {
        articles = [{
          ...data,
          image: data.thumbnail || 'https://via.placeholder.com/600x400',
          date: data.date
        }];
      }
    }
